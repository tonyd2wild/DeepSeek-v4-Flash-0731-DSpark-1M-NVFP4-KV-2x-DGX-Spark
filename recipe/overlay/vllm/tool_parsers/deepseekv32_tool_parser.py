...
class DeepSeekV32ToolParser(ToolParser):
    ...
    def extract_tool_calls_streaming(
        ...
        if content:
            ...
            # carries tools. That key is absent from the OpenAI wire format, and
            # an empty array is truthy in JavaScript, so clients written as
            #     if (delta.tool_calls) {...} else if (delta.content) {...}
            # route every text chunk into the tool-call branch and silently drop
            # the answer. Observed with VS Code Insiders' chat agent: complete
            # 2135-character reply on the wire, "Sorry, no response was returned"
            # in the UI. Omitting the field restores the documented shape.
